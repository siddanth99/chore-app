      ...
      <div className="flex items-center">
        {[1, 2, 3, 4, 5].map((star) => (
          <span
            key={star}
              className={`text-lg ${
              star <= score ? 'text-yellow-400' : 'text-slate-400 dark:text-slate-600'
            }`}
          >
            ★
          </span>
        ))}
        <span className="ml-2 text-sm text-slate-500 dark:text-slate-400">({score}/5)</span>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-slate-950 py-12 px-4 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-4xl">
        ...
        {averageRating.count > 0 && (
          <Card className="mb-6">
            <h2 className="text-xl font-semibold text-slate-800 dark:text-slate-100 mb-4">Rating Summary</h2>
                ...
                {ratings.map((rating) => (
                  <div key={rating.id} className="border-b border-gray-200 dark:border-slate-700 pb-4 last:border-0">
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <p className="font-medium text-slate-900 dark:text-slate-50">{rating.fromUser.name}</p>
                        <Link
                ...
                {completedChores.map((chore) => (
                  ...
                  >
                    <Link
                      ...
                      className="text-lg font-medium text-slate-900 dark:text-slate-50 hover:text-blue-600 dark:hover:text-blue-400 transition-colors"
                    >
                      {chore.title}
                    </Link>
        ...
        {ratings.length === 0 && averageRating.count === 0 && (
          <Card className="mb-6 text-center">
            <div className="py-8">
              ...
              <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-50 mb-2">No ratings yet</h3>
              <p className="text-slate-500 dark:text-slate-400">This user hasn't received any ratings yet.</p>
            </div>
          </Card>
        )}
        ...
        {completedChores.length === 0 && (
          <Card className="text-center">
            <div className="py-8">
              <div className="text-5xl mb-4">📋</div>
              <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-50 mb-2">No completed chores</h3>
