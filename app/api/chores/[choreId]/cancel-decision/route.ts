import { NextRequest, NextResponse } from 'next/server'
import { requireRole } from '@/server/auth/role'
import { UserRole } from '@prisma/client'
import { decideCancellation } from '@/server/api/cancellations'

export async function POST(
  request: NextRequest,
  context: { params: Promise<{ choreId: string }> }
) {
  try {
    const customer = await requireRole(UserRole.CUSTOMER)
    const { choreId } = await context.params
    const body = await request.json()
    const { decision } = body

    if (decision !== 'APPROVE' && decision !== 'REJECT') {
      return NextResponse.json(
        { error: 'Decision must be either APPROVE or REJECT' },
        { status: 400 }
      )
    }

    const result = await decideCancellation(choreId, customer.id, decision)

    return NextResponse.json({
      chore: result.chore,
      cancellationRequest: result.request,
    })
  } catch (error: any) {
    console.error('Error deciding cancellation:', error)
    
    if (error.message.includes('not found')) {
      return NextResponse.json({ error: error.message }, { status: 404 })
    }
    
    if (error.message.includes('Only the chore owner')) {
      return NextResponse.json({ error: error.message }, { status: 403 })
    }
    
    if (error.message.includes('must be in CANCELLATION_REQUESTED') || error.message.includes('No pending')) {
      return NextResponse.json({ error: error.message }, { status: 400 })
    }

    return NextResponse.json(
      { error: error.message || 'Failed to process cancellation decision' },
      { status: 400 }
    )
  }
}

