...
export async function listPublishedChoresWithFilters(
  ...
  // Only add budget filters when min/max are provided
  // This ensures chores with null budget are included when no filter is active
  if (filters.minBudget != null || filters.maxBudget != null) {
    const budgetConditions: any[] = [];
    if (filters.minBudget != null) {
      budgetConditions.push({ budget: { gte: Number(filters.minBudget) } });
    }
    if (filters.maxBudget != null) {
      budgetConditions.push({ budget: { lte: Number(filters.maxBudget) } });
    }
    if (budgetConditions.length > 0) {
      where.AND = where.AND || [];
      where.AND.push(...budgetConditions);
    }
  }
