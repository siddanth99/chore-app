...
export function EnhancedChoreCard({
  ...
  const category = CATEGORIES.find(c => c.id === chore.category.toLowerCase()) || CATEGORIES[0];
        ...
        <div className="flex items-center justify-between pt-3 border-t border-border/50">
          ...
          {chore.budget ? (
            <span className="px-4 py-1.5 rounded-full bg-accent/20 text-accent font-bold text-sm">
              {chore.currency || '$'}{chore.budget}
            </span>
          ) : (
            <span className="text-sm text-muted-foreground italic">Budget flexible</span>
          )}
