...
export function CategorySelect({ 
  ...
  value: string; // Category ID/slug
  onChange: (value: string) => void; // Returns category ID
  disabled?: boolean;
  categories?: Array<{ id: string; label: string; icon: React.ElementType; color: string }>; // Allow server-provided categories
}) {
  const categoryList = categories || CATEGORIES;
  // Find selected category by ID first, then by label (for backward compatibility)
  const selectedCategory = categoryList.find(c => c.id === value || c.label.toLowerCase() === value.toLowerCase());
  
  // Map suggested category labels to category IDs
  const suggestedCategoryIds = SUGGESTED_CATEGORIES.map(label => {
    const cat = categoryList.find(c => c.label.toLowerCase() === label.toLowerCase());
    ...
    <motion.div variants={itemVariants} className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2">
        {suggestedCategoryIds.map((cat) => {
          const isSelected = value === cat.id || value.toLowerCase() === cat.label.toLowerCase();
          const categoryData = categoryList.find(c => c.id === cat.id);
      ...
      <div className="relative">
        <Input
          type="text"
          value={selectedCategory?.label || value}
          onChange={(e) => {
            const inputValue = e.target.value;
            // Try to find matching category by label
            const matched = categoryList.find(c => c.label.toLowerCase() === inputValue.toLowerCase());
            // If match found, use ID; otherwise use the input value (custom category)
            onChange(matched ? matched.id : inputValue);
          }}
